...
Pour $p=1$, on ne peut pas vraiment dire que l'hypothèse d'indépendance soit vérifiée. Les simulations montrent que les distributions sont affectées par le type particulier de préférences, et la limite fluide n'est pas très précise, en particulier aux limites ($K$ proche de $1$ ou de $n$). On peut entre autres remarquer que la probabilité de célibat $1/n$ donnée par l'@eq:f_sr_k est clairement sur-estimée si $n$ est pair (puisqu'elle est alors nulle en réalité), mais exacte pour $n$ impair. La limite fluide réussit quand même à donner le comportement en $1/K$ commun à toutes les préférences considérées. De ce point de vue, elle est meilleure que l'équation récursive #ref(<eq:s_approx>, supplement: none) (dont elle dérive pourtant), qui donne $S_R (K) = delta_K^1$ pour $p=1$.
